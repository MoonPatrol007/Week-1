...
package javaReview0102;

/**
 * Part 02 of Java Review 1 CSIS 2450 Software Engineering I
 * 
 * @author hector_juarez
 *
 */
public class javaReview0102 {
	...
	public static void main(String[] args) {
		...
		printLine(145);
		System.out.print(" Customer Payment History\n");

		printLine(145);
		System.out.print("\tName\t      Account\t01\t02\t03\t04\t05\t06\t07\t08\t09\t10\t11\t12\tStanding");
		System.out.println();

		printLine(145);
		@SuppressWarnings("unused")
		int checkAccount = 0;
		int count = 0; // get account number from user

		for (int i = 0; i < payments.length; ++i) {
			for (int j = 0; j <= payments[i].length - 1; j++) {

				// check if monthly payment zero present
				if (payments[i][j] == 0) {
					count++;
				}
				...
				if (j == 0) {
					...
					// line index 00 for both string and double
					String newIndent = customers[i];
					int notDouble = (int) payments[i][j];
					System.out.format("%17s", newIndent); 
					System.out.print("\t" + notDouble + "\t");															// array

				} else {
					System.out.print(payments[i][j] + "\t"); // lines index 01 to 12
